...
export class CardBattleRoom {
  ...
  pushLog(entry) { this.room.battleLog = [entry, ...(Array.isArray(this.room.battleLog) ? this.room.battleLog : [])].slice(0, 60); }
  async ensureTeams(force = false) {
    ...
    for (const id of ["p1", "p2"]) {
      ...
      if ((id === "p1" && p1Needs) || (id === "p2" && p2Needs)) {
        player.hand = hand.map(addCardHealth); player.team = player.hand.map(addCardHealth); player.activeIndex = 0; player.card = addCardHealth(player.team[0]); player.ready = false;
      }
      if (force) { player.card = addCardHealth(player.team[0]); player.activeIndex = 0; player.ready = false; }
    }
    if (force) { this.room.selectedStat = null; this.room.lastWinner = null; this.room.lastDamage = 0; this.room.lastEvent = "deal"; this.room.knockout = false; this.room.battleActive = false; this.room.matchWinner = null; this.room.rewardSummary = null; this.room.battleLog = []; this.room.lastResult = "Fresh 3-card teams dealt. Pick a lead card, then both players press Ready."; }
  }
  ...
  async handleMessage(socket, data) {
    ...
    if (message.type === "playCard") {
      ...
      const idx = (player.team || player.hand || []).findIndex(candidate => candidate.id === cardId);
      ...
      this.room.selectedStat = null; this.room.lastWinner = null; this.room.lastDamage = 0; this.room.lastEvent = "switch";
      if (this.room.battleActive) {
        this.room.lastResult = `${player.name} switched to ${player.card.name}. Player 1 controls the next roll.`;
        this.pushLog({ round: this.room.round, title: "Goober switch", text: `${player.name} switched active Goober to ${player.card.name}.`, p1Hp: `${this.room.players.p1?.card?.hp || 0}/${this.room.players.p1?.card?.maxHp || 0}`, p2Hp: `${this.room.players.p2?.card?.hp || 0}/${this.room.players.p2?.card?.maxHp || 0}` });
      } else {
        player.ready = false;
        this.room.lastResult = `${player.name} chose a lead card. Waiting for both players to press Ready.`;
      }
    }
  ...
  rollBattle() {
    ...
    const battleStat = BATTLE_STATS[cryptoRandomInt(0, BATTLE_STATS.length - 1)], matchup = getBattleMatchup(p1.card, p2.card, battleStat);
    const attackerId = matchup.winnerId, defenderId = attackerId === "p1" ? "p2" : "p1";
    const attacker = this.room.players[attackerId], defender = this.room.players[defenderId], defenderStartingHp = defender.card.hp;
